...
async function getBumpType(packageName) {
    ...
    for await (const commit of commits) {
        ...
        // with BREAKING CHANGE or by having a ! in the header after the type 
        // From https://www.conventionalcommits.org/en/v1.0.0:
        // "BREAKING CHANGE: a commit that has a footer BREAKING CHANGE:, or appends a ! after the type, introduces a breaking API change (correlating with MAJOR in Semantic Versioning). A BREAKING CHANGE can be part of commits of any type."
        const majorBecauseHeaderHasBang = commit.header.includes(`!:`)
